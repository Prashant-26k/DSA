...
make two head pointers for both list and create an empty list and also a current pointer for this list , calculate values from both using :
(expression)? true : false, then  calculate carry and create next as a new node with sum%10 as value and update current pointer. update head pointers after checking they are not null. after the loop check for carry and if present then just add it to the next. in the end return list.next (the empty list at the begining).
