...
Calculate the maximum number and then make a frequency array and update its values then for k in range 2 to size check and compare first and last numbers for frequency and then return accordingly, for k = 1 return max number in array with freq 1 and  for k = size return maxNum.
